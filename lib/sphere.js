...
//	this.holder.innerHTML = "";
	...
	this.addWheelListener( self.holder, function(event){self.onMouseWheel(event, self); }, false );
...
Photosphere.prototype.startMoving = function(){
	...
	this.interval = setInterval(function(){
		if(self.enableAutoMove) {
			...
			else if(self.lat < self.centerPoint.lat) { self.lat += 0.04;  }
...
Photosphere.prototype.onMouseWheel = function( event, self ) {
	...
	proposed = self.camera.fov + event.deltaY * 0.5;
...
Photosphere.prototype.loadTexture = function( image ) {
...
Photosphere.prototype.loadEXIF = function(callback){
	// We have to resize the max width of the picture to the
	// max supported texture width of the GPU in case we are
	// in WebGL mode:
	ratio = 1.0;
	max_size = -1;
	if(this.canDoWebGL()){
		canvas  = document.createElement('canvas');
		ctx     = canvas.getContext('webgl') || canvas.getContext('experimental-webgl');
		max_size = ctx.getParameter(ctx.MAX_TEXTURE_SIZE);
		console.log("Max texture size: " + max_size);
	}
	
	if(this.exif != undefined){
		if(max_size > 0) {
			ratio = max_size/this.exif['full_width'];
			ratio = ratio > 1.0 ? 1.0 : ratio;
			
			for(key in this.exif) {
				this.exif[key] *= ratio;
			}
		}
	...
	this.loadBinary(function(data){
		...
		if(max_size > 0) {
			ratio = max_size/getAttr("GPano:FullPanoWidthPixels");
			ratio = ratio > 1.0 ? 1.0 : ratio;
		}

		// Check https://developers.google.com/panorama/metadata/
		// for an explanation of what those parameters mean
		// including what parameters have to be scaled:
		self.exif = {
			"full_width"	: ratio*parseInt(getAttr("GPano:FullPanoWidthPixels")),
			"full_height"	: ratio*parseInt(getAttr("GPano:FullPanoHeightPixels")),
			"crop_width"	: ratio*parseInt(getAttr("GPano:CroppedAreaImageWidthPixels")),
			"crop_height"	: ratio*parseInt(getAttr("GPano:CroppedAreaImageHeightPixels")),
			"x" 			: ratio*parseInt(getAttr("GPano:CroppedAreaLeftPixels")),
			"y" 			: ratio*parseInt(getAttr("GPano:CroppedAreaTopPixels"))
		}
...
Photosphere.prototype.addWheelListener = function( elem, callback, useCapture ) {
	// Cross-browser mousewheel event hook adapted from Mozilla Developer Network
	...
	this._addWheelListener( elem, support, callback, useCapture, support );
	
	// handle MozMousePixelScroll in older Firefox
	if( support == "DOMMouseScroll" ) {
		this._addWheelListener( elem, "MozMousePixelScroll", callback, useCapture, support );
	}
};

Photosphere.prototype._addWheelListener = function( elem, eventName, callback, useCapture, support ) {
